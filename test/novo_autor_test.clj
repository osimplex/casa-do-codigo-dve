...
#_(- Cria o payload-reader e mantém aberto com o with-open
     - Cria um deflow
...
(api/defflow cria-novo-autor
  {:init (constantly (cria-contexto-com-payload-e-dados payload #{}))}
  [resposta
   (api/get-state
    (:enter (novo-autor/handler
             {:datomic {:db #{} :funcao-transacao executa-transacao-datomic}})))
   :let [body (get-in resposta [:response :body])]]
  (match? {:status 200} (:response resposta))
  (match? true (contains? body :id)))

(api/defflow nao-pode-criar-autor-com-email-duplicado
  {:init (constantly (cria-contexto-com-payload-e-dados payload dados-com-email-duplicado))}
  [resposta
   (api/get-state
    (:enter (novo-autor/handler
             {:datomic {:db dados-com-email-duplicado
