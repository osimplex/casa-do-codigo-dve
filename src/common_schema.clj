...
(defn- localdate-matcher [schema]
  (if (= schema LocalDate)
    (fn [value] (LocalDate/parse value (DateTimeFormatter/ofPattern "yyyy-MM-dd")))
    nil))

(defn- bigdecimal-matcher [schema]
  (if (= schema java.math.BigDecimal)
    (fn [value] (java.math.BigDecimal. value))
    nil))
...
(defn coerce [schema dados]
  ...
  (let [coercer
        (coerce/coercer
         schema (fn [schema] (or (coerce/json-coercion-matcher schema)
