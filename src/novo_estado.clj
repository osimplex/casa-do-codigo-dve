...
(def handler
  {:name :novo-estado
   :enter
   (fn [context]
     (let [id-pais (Long/parseLong (get-in context [:request :path-params :id-pais]))
       ...
       (cond
         ...
         (not (datomic-lib/busca-entidade dados (:pais/nome id-pais)))
         (utilitarios/respond-validation-error-with-json
          context {:global-erros ["O pais não existe"]})

         (ja-tem-estado-com-mesmo-nome? dados id-pais (:nome payload))
         (utilitarios/respond-validation-error-with-json
          context {:global-erros ["Já tem estado com o mesmo nome para este país"]})

         :else (let [novo-id
                     (utilitarios/executa-transacao
                      context [(datomic-schema-estado/to-schema
                                (assoc payload :id-pais id-pais))])]
