...
(defn converte-livro-saida [livro]
  {:titulo (:livro/titulo livro)
   ...
   :autor {:nome (get-in livro [:livro/id-autor :autor/nome])}
   :categoria {:nome  (get-in livro [:livro/id-categoria :categoria/nome])}})

(defn- busca-por-id [context id]
  (let [dados (get-in context [:request :db])]
    (d/pull dados '[:livro/titulo
                    :livro/resumo
                    :livro/preco
                    :livro/isbn
                    :livro/data-lancamento
...
(def handler
  {:name :detalhe-livro
   :enter (fn [context]
            (let [id-livro (Long/parseLong
                            (get-in context [:request :path-params :id]))
