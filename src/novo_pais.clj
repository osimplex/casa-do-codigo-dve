  ...
  {:name :novo-pais
   :enter (fn [context]
            (let [payload (utilitarios/parse-json-body context)
                  ...
                  ;deve ter outro jeito de criar uma funcao para atrasar a
              ...
              (cond
                ...
                (datomic-lib/busca-entidades-por-unico-atributo
                 dados :pais/nome (:nome payload))
                (utilitarios/respond-validation-error-with-json
                 context {:global-erros ["Já existe um pais com o mesmo nome"]})

                :else (let [novo-id (utilitarios/executa-transacao
                                     context [(datomic-schema-pais/to-schema payload)])]
