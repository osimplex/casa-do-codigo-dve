...
(defn ja-existe-email-cadastrado [db autor]
  (let [email (:email autor)
        ;deve ter um jeito de passar um simbolo que represente que eu quero o valor da
        ;variável email quando eu passo a string direto funciona, acho que é isso.
    ...
    ;eu tinha feito assim => ((not (empty? emails))) a IDE mandou eu fazer assim
    (seq emails)))
...
(s/defn ^:always-validate logica-cria-autor [context
                                             payload :- NovoAutorHandlerRequest
                                             db
                                             funcao-transacao]
  (cond
    (ja-existe-email-cadastrado db payload)
    (utilitarios/respond-validation-error-with-json
     context {:global-erros ["Ja existe autor com email cadastrado"]})
...
(defn handler [{:keys [:datomic]}]
  {:name :novo-autor
   ...
   (fn [context]
     ...
     #_(- valida com o mali o payload externo
     ...
     (let [payload (get-in context [:request :json-params])
       ...
       (cond
         ...
         :else (logica-cria-autor
                context payload (:db datomic) (:funcao-transacao datomic)))))})
