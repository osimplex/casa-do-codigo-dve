...
(defn busca-entidade
  ...
  ;aqui eu vou trocar para usar a api de query.
  ;aí vou ter que explorar mais o datomic.Database
  [dados id]
  ;pelo que entendi, aqui busca uma entidade que tem exatamente este id
  (let [query '[:find ?id-entidade
                :in $ ?id-entidade
                :where [?id-entidade]]
...
(defn busca-entidades-por-unico-atributo [dados atributo-match valor-buscado]
  (let [query '[:find ?e
    ...
    ;nao da para simplesmente retornar os livros, pq ele um hashset.
    ;por mais que esteja vazio, não é nil. se não é nil, é true
    (seq entidades)))
