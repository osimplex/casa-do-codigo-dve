   ...
   :enter (fn [context]
            (let [payload (get-in context [:request :json-params])
              ...
              (cond
                ...
                (ja-existe-nome-cadastrado context payload)
                (utilitarios/respond-validation-error-with-json
                 context {:global-errors ["Ja existe categoria com o nome passado"]})

                :else
                (let [novo-id
                      (utilitarios/executa-transacao
                       context [(datomic-schema-categoria/categoria-to-schema payload)])]
